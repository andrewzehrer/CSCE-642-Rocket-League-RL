...
from rocket_league_rl.rlgym_ppo.batched_agents import BatchedTrajectory, comm_consts
from rocket_league_rl.rlgym_ppo.batched_agents.batched_agent import batched_agent_process
from rocket_league_rl.rlgym_ppo.util import WelfordRunningStat
...
class BatchedAgentManager(object):
    ...
    def set_render(self, enable: bool, proc_id: int = 0):
        """
        Toggle rendering for a specific child process at runtime.
        :param enable: True to enable rendering, False to disable.
        :param proc_id: Process id to target (default 0).
        :return: None
        """
        process, parent_end, child_endpoint, shm_view = self.processes[proc_id]
        msg = comm_consts.RENDER_TOGGLE_HEADER + [1.0 if enable else 0.0]
        parent_end.sendto(comm_consts.pack_message(msg), child_endpoint)
