...
from rocket_league_rl.rlgym_ppo.batched_agents import BatchedAgentManager
from rocket_league_rl.rlgym_ppo.ppo import ExperienceBuffer, PPOLearner
from rocket_league_rl.rlgym_ppo.util import WelfordRunningStat, reporting, torch_functions, KBHit
...
class Learner(object):
    def __init__(
        ...
        if checkpoints_save_folder is None:
            checkpoints_save_folder = os.path.join("data", "checkpoints", "rlgym-ppo-run")
        ...
        obs_space_size, act_space_size, action_space_type = self.agent.init_processes(
            ...
            render=(render and not render_last_only),
    ...
    def _learn(self):
        ...
        while self.agent.cumulative_timesteps < self.timestep_limit:
            ...
            # If requested, enable rendering only for the final iteration.
            render_was_enabled = False
            remaining = self.timestep_limit - self.agent.cumulative_timesteps
            if self.render_last_only and self.render and remaining <= self.ts_per_epoch:
                try:
                    self.agent.set_render(True)
                    render_was_enabled = True
                except Exception:
                    print("Failed to enable render for final iteration")

            experience, collected_metrics, steps_collected, collection_time = self.agent.collect_timesteps(self.ts_per_epoch)

            # Turn rendering back off after the final iteration.
            if render_was_enabled:
                try:
                    self.agent.set_render(False)
                except Exception:
                    print("Failed to disable render after final iteration")
