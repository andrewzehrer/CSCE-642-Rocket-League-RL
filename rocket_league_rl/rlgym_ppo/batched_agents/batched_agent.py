def batched_agent_process(proc_id, endpoint, shm_buffer, shm_offset, shm_size, seed, render, render_delay):
    ...
    from rocket_league_rl.rlgym_ppo.batched_agents import comm_consts

    # Default no-op implementations so functions exist even if rendering
    # is disabled at startup. This allows rendering to be toggled at
    # runtime without causing UnboundLocalError.
    ...
    if render:
        try:
            from rlviser_py import get_game_paused as _rgp, get_game_speed as _rgs
            get_game_paused = _rgp
            get_game_speed = _rgs
        except Exception:
            # Keep defaults if rlviser_py isn't available
            pass
    ...
    try:
        ...
        while True:
            ...
            elif header[0] == comm_consts.RENDER_TOGGLE_HEADER[0]:
                # Toggle rendering on/off at runtime. Expect a single float after header:
                # 1.0 -> enable render, 0.0 -> disable render
                data = message[header_len:]
                try:
                    enable = bool(int(data[0]))
                except Exception:
                    enable = False
                render = enable
